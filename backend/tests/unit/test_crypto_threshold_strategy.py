...
def test_strategy_observes_touch_markets_until_barrier_model_exists() -> None:
    strategy = CryptoThresholdStrategy(min_edge=Decimal("0.07"))

    signal = strategy.evaluate(
        CryptoMarketContext(
            market_id="market-3",
            question="Will BTC hit $120,000 before July 31, 2026?",
            now=datetime(2026, 7, 1, tzinfo=timezone.utc),
            deadline=datetime(2026, 7, 31, tzinfo=timezone.utc),
            current_price=Decimal("100000"),
            annualized_volatility=Decimal("0.45"),
