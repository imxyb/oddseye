...
@pytest.mark.asyncio
async def test_buy_signal_can_create_filled_paper_order_at_conservative_ask(tmp_path) -> None:
    sessionmaker = create_sessionmaker(f"sqlite+aiosqlite:///{tmp_path / 'signal-order.db'}")
    async with sessionmaker.bind.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    async with sessionmaker() as session:
        venue = Venue(code="POLYMARKET", name="Polymarket")
        ...
        event = PredictionEvent(
            venue_id=venue.id,
            external_event_id="signal-order-event",
            protocol="POLYMARKET",
        ...
        market = PredictionMarket(
            event_id=event.id,
            venue_id=venue.id,
            external_market_id="signal-order-market",
            protocol="POLYMARKET",
        ...
        snapshot = MarketSnapshot(
            ...
            outcome1_best_ask=Decimal("0.52"),
            outcome1_spread=Decimal("0.03"),
        ...
        session.add_all([snapshot, signal])
        await session.commit()

        response = await create_order_from_signal(
            session,
            signal_id=signal.id,
            account_id=account.id,
            notional=Decimal("5"),
        ...
        assert response["order"]["signal_id"] == signal.id
        assert response["fill"]["snapshot_id"] == snapshot.id
        assert response["fill"]["price"] == 0.50125
        assert response["position"]["mark_price"] == 0.48
    await sessionmaker.bind.dispose()


@pytest.mark.asyncio
async def test_signal_order_rejects_legacy_zero_price_buy_signal(tmp_path) -> None:
    sessionmaker = create_sessionmaker(f"sqlite+aiosqlite:///{tmp_path / 'zero-signal-order.db'}")
    async with sessionmaker.bind.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    async with sessionmaker() as session:
        venue = Venue(code="POLYMARKET", name="Polymarket")
        ...
        event = PredictionEvent(
            venue_id=venue.id,
            external_event_id="zero-signal-event",
            protocol="POLYMARKET",
            question="Will BTC be above $80,000?",
            categories=["crypto"],
            status="OPEN",
        ...
        market = PredictionMarket(
            event_id=event.id,
            venue_id=venue.id,
            external_market_id="zero-signal-market",
            protocol="POLYMARKET",
            question=event.question,
            status="OPEN",
            closes_at=event.closes_at,
        ...
        session.add(
            MarketSnapshot(
                ...
                ts=datetime.now(UTC),
                outcome0_label="Yes",
                outcome0_best_bid=Decimal("0.01"),
                outcome0_best_ask=Decimal("0.02"),
        ...
        with pytest.raises(ValueError, match="signal is not orderable"):
