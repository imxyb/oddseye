...
describe("buildSettingsInfoRows", () => {
  it("includes API base URL and refresh cadence rows", () => {
    ...
    expect(rows).toEqual([
      ...
      { label: "Radar refresh", value: "Pull to refresh; workers update hot markets about every 5 min" },
      { label: "Signal refresh", value: "Workers recompute signals about every 5 min" },
    ]);
  });
});
